 ...
 */
var ConnectedDetailView = (function (SiftView) {
  function ConnectedDetailView() {
    SiftView.call(this);
    this.registerOnLoadHandler(this.onLoad.bind(this));
  }

  if ( SiftView ) ConnectedDetailView.__proto__ = SiftView;
  ConnectedDetailView.prototype = Object.create( SiftView && SiftView.prototype );
  ConnectedDetailView.prototype.constructor = ConnectedDetailView;

  // TODO: link to docs
  ConnectedDetailView.prototype.presentView = function presentView (value) {
    ...
    Object.keys(secondary).forEach(function (k) {
      var ul = document.getElementById(k + '-connections');
      if(secondary[k].length > 0) {
        ...
        for(var i = 0; i < secondary[k].length; i++) {
  ...
  // TODO: link to docs
  ConnectedDetailView.prototype.willPresentView = function willPresentView (value) { };

  ConnectedDetailView.prototype.onLoad = function onLoad () {
    var this$1 = this;

    var logos = document.getElementsByClassName('social-logo');
    for (var i = 0; i < logos.length; i++) {
      logos[i].addEventListener('mouseenter', this$1._onMouseEnter.bind(this$1));
      logos[i].addEventListener('mouseleave', this$1._onMouseLeave.bind(this$1));
    }
  };

  ConnectedDetailView.prototype._onMouseEnter = function _onMouseEnter (ev) {
    var logos = document.getElementsByClassName('social-logo');
    for (var i = 0; i < logos.length; i++) {
  ...
  ConnectedDetailView.prototype._onMouseLeave = function _onMouseLeave (ev) {
    var logos = document.getElementsByClassName('social-logo');
    for (var i = 0; i < logos.length; i++) {
      logos[i].classList.remove('opaque');
      logos[i].classList.remove('zoom');
    }
    var cls = document.getElementsByClassName('connection-list');
    for (var i$1 = 0; i$1 < cls.length; i$1++) {
      cls[i$1].style.display = 'none';
    }
    document.getElementById('importance').style.display = '';
  };

  return ConnectedDetailView;
}(SiftView));
