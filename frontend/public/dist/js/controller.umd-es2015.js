 ...
 */
var ConnectedController = (function (SiftController) {
  ...
  if ( SiftController ) ConnectedController.__proto__ = SiftController;
  ConnectedController.prototype = Object.create( SiftController && SiftController.prototype );
  ConnectedController.prototype.constructor = ConnectedController;

  // TODO: link to docs
  ConnectedController.prototype.loadView = function loadView (state) {
  ...
  ConnectedController.prototype._loadSummaryView = function _loadSummaryView () {
    return {
      html: 'summary.html',
      data: Promise.all([this._getWebhooks(), this._getAuths(), this._getStats(), this._getConnectedThreads()]).then(function (results) {
        return { webhooks: results[0], auths: results[1], stats: results[2], threads: results[3] };
      })
    };
  };

  ConnectedController.prototype._getWebhooks = function _getWebhooks () {
  ...
  ConnectedController.prototype._getAuths = function _getAuths () {
    return this.storage.get({
      bucket: 'auth',
  ...
  ConnectedController.prototype._getStats = function _getStats () {
    return this.storage.getAll({
      bucket: 'stats'
    }).then(function (results) {
      var ret = {angel_list: {}, twitter: {}};
      results.forEach(function (r) {
        if(r.value) {
          var jv = JSON.parse(r.value);
  ...
  ConnectedController.prototype._getConnectedThreads = function _getConnectedThreads () {
    return this.storage.getAll({
      bucket: '_email.tid'
    }).then(function (results) {
      var ret = {high: 0, medium: 0};
      results.forEach(function (r) {
        if(r.value) {
          var jv = JSON.parse(r.value);
  ...
  ConnectedController.prototype._loadDetailView = function _loadDetailView (data) {
    return {html: 'detail.html', data: data};
  };

  ConnectedController.prototype.onStorageUpdate = function onStorageUpdate (updated) {
    var this$1 = this;

    updated.forEach(function (bucket) {
      switch (bucket) {
        case 'auth':
          this$1._getAuths().then(function (auths) {
            this$1.publish('auth', auths);
          });
          break;
        case 'stats':
          this$1._getStats().then(function (stats) {
            this$1.publish('stats', stats);
          });
          break;
        case '_email.tid':
          this$1._getConnectedThreads().then(function (threads) {
            this$1.publish('threads', threads);
          });
          break;
      }
    });
